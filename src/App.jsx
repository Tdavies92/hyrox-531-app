...
// IronPath 5/3/1 — v7 (clean)
// - 4-day split
// - Cycle + Week selectors
// - Local log with remove/export; est 1RM (Epley)
...
export default function App() {
  ...
  const exportCSV = () => {
    ...
    const rows = log.map(e => [
      ...
      e.cycle,
      e.week,
      e.absWeek ?? absWeek(e.cycle, e.week),
      e.lift,
      e.units,
      (e.tm ?? ''),
      (e.topSet?.pct ?? ''),
      (Number.isFinite(e.topSet?.load)? e.topSet.load.toFixed(1): ''),
      (e.topSet?.repsTarget ?? ''),
      (typeof e.topSet?.repsActual==='number'? e.topSet.repsActual: ''),
...
')
    ...
    const url = URL.createObjectURL(blob); const a = document.createElement('a');
    a.href = url; a.download = 'ironpath_log.csv'; a.click(); URL.revokeObjectURL(url)
  }
          ...
          {days.map((d, idx)=> (
            <div key={d.lift.key} style={dayBox}>
              ...
              <div style={{ marginTop:8, display:'flex', gap:8, flexWrap:'wrap' }}>
                {d.sets.map((s,i)=> (<div key={i} style={{...pill, background:d.lift.color+"20"}}>{`S${i+1}: ${s.disp.toFixed(1)} ${units} @ ${(s.pct*100).toFixed(0)}% × ${reps[i] ?? reps[reps.length-1]}`}</div>))}
              </div>
