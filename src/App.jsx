...
/****************************
 * IronPath 5/3/1 — 4‑day split (v6.2 full)
 * - Cycle + Week selectors (1..99 / 1..4)
 * - Local log with remove/export; est 1RM (Epley) shown & saved
 * - Per-lift charts (no Combined chart)
 * - CSV export
...
function ChartSVG({ title, units, series, xMax, height=140 }) {
  ...
  const buildPath = (pts)=> pts.filter(p=>Number.isFinite(p.y)).sort((a,b)=>a.x-b.x).map((p,i)=>`${i?'L':'M'}${sx(p.x)},${sy(p.y)}`).join(' ');
  return (
...
export default function App() {
  ...
  const saveTopSet = (dayIndex, actualReps) => {
    ...
    const entry = {
      ...
      cycle, week: weekInCycle, absWeek: absWeek(cycle, weekInCycle),
      lift: d.lift.label, tm: d.tmDisp, units,
      topSet: { pct: (top.pct*100).toFixed(0)+'%', load: top.disp, units,
  ...
  const exportCSV = () => {
    ...
    const rows = log.map(e => [ e.ts, e.cycle, e.week, e.absWeek ?? absWeek(e.cycle, e.week), e.lift, e.units,
      (e.tm ?? ''), (e.topSet?.pct ?? ''), (Number.isFinite(e.topSet?.load)? e.topSet.load.toFixed(1): ''),
      (e.topSet?.repsTarget ?? ''), (typeof e.topSet?.repsActual==='number'? e.topSet.repsActual: ''),
      (Number.isFinite(e.topSet?.est1RM)? e.topSet.est1RM.toFixed(1): '') ]);
    const csv = [header, ...rows].map(r => r.map(x => { const s=String(x ?? ''); return (s.includes(',')||s.includes('"')||s.includes('
')) ? '"'+s.replace(/"/g,'""')+'"' : s; }).join(',')).join('
');
    const blob = new Blob([csv], { type:'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download='ironpath_log.csv'; a.click(); URL.revokeObjectURL(url);
  };
  ...
  function seriesForLift(label) {
    ...
    for (let x=1; x<=maxX; x++) {
      const cands = entries.filter(e => (e.absWeek ?? absWeek(e.cycle,e.week))===x);
          ...
          {log.slice(0,50).map((e)=> (
            <div key={e.id} style={logRow}>
              <div style={{ display:'flex', justifyContent:'space-between', gap:8, alignItems:'baseline', flexWrap:'wrap' }}>
                <div><strong>{e.lift}</strong> • Cycle {e.cycle}, Week {e.week} • {new Date(e.ts).toLocaleString()}</div>
